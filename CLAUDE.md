...
- インフラは AWS + Terraform（本番デプロイ構成は ADR 0009 で確定済み）
- 運用基盤（運用ルール・CI・ADR）に続き、Backstage アプリ本体（`backstage/`）と
  ゴールデンパステンプレート（`backstage/templates/service-baseline/`）を実装済み。
  ローカル開発の基準は guest 認証 + インメモリ SQLite（ADR 0003 参照）
- 本番は ECS Fargate + Aurora Serverless v2 + GitHub OAuth。常時稼働させず、
  検証時のみ apply → destroy する 3 層 state 分離（`terraform/`、ADR 0009 参照）
...
| `.github/workflows/**` を変える | `docs/operations/github-flow-guardrails.md` と `docs/operations/branch-protection.md`（required status checks との整合） |
...
helper は PR を通常 PR として作成する。draft PR にはしない。
...
ADR で判断が変わった場合は、影響する領域の正本も同じ PR で更新する。
