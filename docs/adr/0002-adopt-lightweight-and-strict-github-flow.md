...
運用ルールの具体（必須ラベル、`Closes / Fixes / Refs` 記法、厳密運用の判定基準）の正本は `CONTRIBUTING.md` とする。CI（PR Policy Check / Commitlint / Markdown Lint / Gitleaks）と helper scripts（`scripts/github/`）も既存 2 リポジトリから移植する。
...
- [CONTRIBUTING.md](../../CONTRIBUTING.md) - 運用ルールの正本
