...
- `.github/workflows/dependency-audit.yml` は `Dependency Audit` / `npm Dependency Audit (root)` /
  `npm Dependency Audit (skeleton)` / `Yarn Resolutions Registry` / `Yarn Resolutions Inventory` /
  `npm Overrides Registry` / `npm Overrides Inventory` の
  7 check run で構成する（Issue #255 / #257 / #263 / #265）。いずれも required status checks には追加しない。
  ...
  - `npm Dependency Audit` はルートと skeleton を対象とする matrix job のため、check run 名は
    `npm Dependency Audit (<label>)` の形になる。fail 要因（新規公開 CVE）が PR の変更内容と
    無関係に発生する点は `Dependency Audit` と同じで、非 required のまま運用する
  - `npm Overrides Inventory` は `Yarn Resolutions Inventory` と同じく schedule / `workflow_dispatch`
    でのみ実行し、PR では skip される。`npm Overrides Registry` は PR でも実行されるが、
    `Yarn Resolutions Registry` と同じく非 required のまま運用する
