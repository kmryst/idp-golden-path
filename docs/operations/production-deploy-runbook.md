...
Actions タブ → **Production Deploy** → `Run workflow`（branch: `main`）。または:
...
Actions タブ → **Production Destroy** → `Run workflow`（branch: `main`、confirm 欄に `destroy` と入力）。または:

```bash
gh workflow run production-destroy.yml --ref main -f confirm=destroy
gh run watch
```
