...
- `ticket-c2c-platform` は、PR Policy Check / Commitlint / Gitleaks / Sync Labels の 4 種を `uses: kmryst/idp-golden-path/.github/workflows/<file>.yml@v1` で消費する薄い caller workflow へ移行済み（ticket-c2c-platform#294〜#297）。`terraform-hannibal` は未移行。
...
Dependabot の除外条件は reusable workflow の job に集約します。消費側の caller workflow は Dependabot PR でも reusable workflow を必ず呼び出し、
branch protection が要求する caller/callee の check 名を作成させます。caller job で先にスキップすると called workflow の job が作成されず、
required status check が欠落して PR がマージ不能になるためです。本リポジトリ自身の Dependabot PR では、同じ job の除外条件が直接実行時にも適用されます。
