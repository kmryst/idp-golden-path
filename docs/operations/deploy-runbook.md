...
workflow は次を自動実行する: イメージ build & push → ipam 層 apply → ephemeral 層 apply →
...
workflow は ephemeral 層 → ipam 層（IPAM も毎回再構築する方針。ADR 0009）の順に destroy し、
...
```bash
terraform -chdir=terraform/ipam init
terraform -chdir=terraform/ipam apply
```
...
terraform -chdir=terraform/ipam destroy
```
