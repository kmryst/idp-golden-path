...
このリポジトリは、Backstage をベースにした Internal Developer Platform (IDP) のポートフォリオ実装
「ゴールデンパスのセルフサービス化」を段階的に進めるためのプロジェクトです。
...
構想・基盤整備の段階から Issue / PR 駆動開発を基本とします。
...
新しい機能追加、テンプレート整備、検証、ドキュメント更新は、原則として Issue から始めます。
...
```bash
git switch -c 12-add-scaffolder-template
```
...
`main` への direct push は branch protection で禁止しています。必ず PR を経由します。
